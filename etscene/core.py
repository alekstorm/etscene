...
# FIXME escape scene data
# TODO examples on first page
...
# FIXME add asynchronous option to etsy-python
    ...
    def __init__(self):
        ...
        Application.__init__(self, {
            ...
            'examples': DictResource({
                'decor': StaticFileResource(os.path.join(STATIC_DIR, 'html', 'example-decor.html')),
                'outfit': StaticFileResource(os.path.join(STATIC_DIR, 'html', 'example-outfit.html')),
            }),
        })

        self.etsy = EtsyV2(callback=lambda: None, api_key=settings.ETSY_KEYSTRING, env=etsy.env.ProductionEnv)
